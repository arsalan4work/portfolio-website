...
export default function ResumeCard({Icon, role, date} :Props) {
  return (
    <div className='mb-6'>
        <div className='flex items-start space-x-6 bg-blue-950/20 transition-all duration-300 p-4 sm:p-8 rounded-md'>
            ...
            <div className='flex-1'>
                    {date && (
                        ...
                        sm:text-lg text-sm font-bold'>{date}</h1>
                    )}
                    <h1 className='text-gray-200 text-xl sm:text-2xl font-semibold'>{role}</h1>
                    <p className='text-gray-300 text-sm sm:text-base pt-3'>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Ab provident eaque voluptatum perferendis fuga consequuntur perspiciatis expedita officia. Hic consequuntur distinctio repellat deleniti.</p>
                </div>
        </div>
    </div>
  )
